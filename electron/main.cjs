...
async function downloadAndInstallGitHubUpdate() {
  ...
  const helperScript = [
    ...
    '}',
    '',
    'function Kill-SxronByName {
    '  try {',
    '    $procs = Get-CimInstance Win32_Process -Filter "Name = \"SXRON Marketplace.exe\"" -ErrorAction SilentlyContinue',
    '    foreach ($p in $procs) { Kill-Tree ([int]$p.ProcessId) }',
    '  } catch {}',
    '  try {',
    '    $procs = Get-CimInstance Win32_Process -Filter "Name = \"sxron-api.exe\"" -ErrorAction SilentlyContinue',
    '    foreach ($p in $procs) { Kill-Tree ([int]$p.ProcessId) }',
    '  } catch {}',
    '}',
    '',
    ...
    'Kill-Tree $ElectronPid',
    'Kill-Tree $ApiPid',
    'Kill-SxronByName',
    '',
    'while ((Get-Date) -lt $deadline) {',
    '  if (Test-ProcessAlive $ElectronPid) { Kill-Tree $ElectronPid }',
    '  if (Test-ProcessAlive $ApiPid) { Kill-Tree $ApiPid }',
    '  Kill-SxronByName',
    '',
    '  $electronFree = Test-FileFree $electronExe',
    '  $apiFree = Test-FileFree $apiExe',
    '  if (-not (Test-ProcessAlive $ElectronPid) -and -not (Test-ProcessAlive $ApiPid) -and $electronFree -and $apiFree) { break }',
    ...
    'if (Get-Process -Name "SXRONLauncher" -ErrorAction SilentlyContinue) { exit 5 }',
    '',
    '# IMPORTANT: this is a full replacement update, not a patch.',
    '# The downloaded installer is outside the installation directory, so it is',
    '# safe to remove the entire old application directory before starting NSIS.',
    'if (Test-Path -LiteralPath $InstallDir) {',
    '  try {',
    '    Remove-Item -LiteralPath $InstallDir -Recurse -Force -ErrorAction Stop',
    '  } catch {',
    '    Start-Sleep -Seconds 2',
    '    try { Remove-Item -LiteralPath $InstallDir -Recurse -Force -ErrorAction Stop } catch { exit 6 }',
    '  }',
    '}',
    '',
    'try {',
