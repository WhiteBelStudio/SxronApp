...
function loadProfile(): ProfileCustomization {
  try { const raw = localStorage.getItem("sxron_profile_customization"); return raw ? { ...DEFAULT_PROFILE, ...JSON.parse(raw) } : DEFAULT_PROFILE; } catch { return DEFAULT_PROFILE; }
}
...
export default function App() {
  ...
  useEffect(() => { try { localStorage.setItem("sxron_profile_customization", JSON.stringify(profile)); } catch { /* storage unavailable */ } }, [profile]);
  useEffect(() => { document.documentElement.dataset.sxronAccent = profile.accent; }, [profile.accent]);

  const goPage = useCallback((nextPage: Page) => { setPage(nextPage); window.scrollTo({ top: 0, behavior: "smooth" }); }, []);
  ...
  if (isMobile) return <div className="mobile-app"><MobileHeader city={city} onCityChange={setCity} /><main className="mobile-app__content">
  ...
  </main><MobileBottomNav page={page} onChange={goPage} /></div>;

  return <div className="desktop-app"><DesktopHeader page={page} city={city} onCityChange={setCity} onNavigate={goPage} onCreate={handleCreate} />
